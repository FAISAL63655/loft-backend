-- 0005_orders_invoices_payments_counters.up.sql
-- الطلبات والفواتير والمدفوعات والعدادات
-- منصة لوفت الدغيري للحمام الزاجل

-- إنشاء أنواع البيانات للطلبات والمدفوعات
CREATE TYPE order_source AS ENUM ('auction', 'direct');
CREATE TYPE order_status AS ENUM (
    'pending_payment', 'awaiting_admin_refund', 'paid', 
    'cancelled', 'refund_required', 'refunded'
);

CREATE TYPE invoice_status AS ENUM (
    'unpaid', 'payment_in_progress', 'paid', 'failed',
    'refund_required', 'refunded', 'cancelled', 'void'
);
...
-- فهارس لعناصر الطلبات
CREATE INDEX idx_order_items_order_id ON order_items(order_id);
CREATE INDEX idx_order_items_product_id ON order_items(product_id);

-- جدول عدادات الفواتير السنوية
...
-- قيد توازي: جلسة دفع حية واحدة لكل فاتورة
CREATE UNIQUE INDEX uq_payments_invoice_live 
...
CREATE OR REPLACE FUNCTION next_invoice_number(target_year INTEGER)
RETURNS TEXT AS $$
DECLARE
    next_seq INTEGER;
    invoice_number TEXT;
BEGIN
    -- إدراج أو تحديث العداد للسنة المحددة
...
BEGIN
    ...
    -- الحصول على إعدادات VAT
    SELECT CAST(value AS BOOLEAN) INTO vat_enabled
    FROM system_settings WHERE key = 'vat.enabled';
    
    SELECT CAST(value AS DECIMAL) INTO vat_rate
    FROM system_settings WHERE key = 'vat.rate';
    
    -- إذا كان VAT معطل، اجعل المعدل صفر
    IF NOT vat_enabled THEN
        vat_rate := 0;
    END IF;
    
    -- الحصول على مدينة المستخدم ورسم الشحن
    ...
    SELECT CAST(value AS NUMERIC) INTO free_threshold
    FROM system_settings WHERE key = 'shipping.free_shipping_threshold';
...
-- إضافة trigger لحساب الإجماليات
CREATE TRIGGER calculate_order_totals_trigger
    BEFORE INSERT OR UPDATE ON orders
    FOR EACH ROW EXECUTE FUNCTION calculate_order_totals();

-- دالة للتحقق من كمية الحمام (يجب أن تكون 1)
CREATE OR REPLACE FUNCTION enforce_pigeon_qty()
RETURNS TRIGGER AS $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM products p 
        WHERE p.id = NEW.product_id AND p.type = 'pigeon'
    ) AND NEW.qty != 1 THEN
...
-- إضافة trigger لفرض كمية الحمام
CREATE TRIGGER enforce_pigeon_qty_trigger
    BEFORE INSERT OR UPDATE ON order_items
    FOR EACH ROW EXECUTE FUNCTION enforce_pigeon_qty();
