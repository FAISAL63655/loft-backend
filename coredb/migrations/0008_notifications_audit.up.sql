-- 0008_notifications_audit.up.sql
-- الإشعارات وسجلات التدقيق
-- منصة لوفت الدغيري للحمام الزاجل

-- إنشاء أنواع البيانات للإشعارات
...
-- فهرس للإشعارات المعلقة للمعالجة
-- ملاحظة: لا يجوز استخدام NOW() في predicate لفهرس جزئي
-- نعتمد فهرس مركب بدون شرط ليستفيد منه الاستعلام: channel,status,next_retry_at
...
-- فهرس للبحث في سجلات التدقيق بالتاريخ
...
BEGIN
    INSERT INTO audit_logs (
        ...
        reason, meta, correlation_id
    ) VALUES (
        p_actor_user_id, p_action, p_entity_type, p_entity_id,
        p_reason, p_meta, p_correlation_id
...
BEGIN
    -- الحصول على مدة الاحتفاظ
    SELECT CAST(value AS INTEGER) INTO retention_days
    FROM system_settings WHERE key = 'notifications.email.retention_days';
    
    -- حذف الإشعارات القديمة
    DELETE FROM notifications 
    WHERE created_at < NOW() - (retention_days || ' days')::INTERVAL
    AND status IN ('sent', 'failed', 'archived');
...
-- دالة لإدارة إعادة المحاولة للإشعارات الفاشلة
CREATE OR REPLACE FUNCTION schedule_notification_retry()
RETURNS TRIGGER AS $$
DECLARE
    backoff_minutes INTEGER;
BEGIN
    -- إذا فشل الإشعار وما زال ضمن حد المحاولات
    IF NEW.status = 'failed' AND NEW.retry_count < NEW.max_retries THEN
        -- حساب وقت المحاولة التالية مع Exponential Backoff
        backoff_minutes := POWER(2, NEW.retry_count) * 5; -- 5, 10, 20 دقيقة
...
-- إضافة trigger لجدولة إعادة المحاولة
CREATE TRIGGER schedule_notification_retry_trigger
    BEFORE UPDATE ON notifications
